...
filtered <- filterAndTrim(fnFs, filtFs, 
                          fnRs, filtRs,
                          truncLen=c(230,210), trimLeft=9, trimRight=21,
...
save(filtFs,file=("output/output_filtFs0.RData")) # Save .RData file
save(filtRs,file=("output/output_filtRs0.RData")) # Save .RData file 
...
rm(derepF); rm(derepR) # remove unneeded objects
save(mergers,file=("output/output_mergers.RData")) # Save in a .RData file

seqtab <- makeSequenceTable(mergers)
saveRDS(seqtab, "output/output_seqtab.rds")
...
physeq2 = phyloseq(OTU, TAX2, MAP)
physeqR <- subset_taxa(physeq2,taxa_sums(physeq2)>1) # Remove any ASV with only 1 read in the dataset
...
############################################
sample_data(physeqR)$InitialConc <- as.numeric(as.character(sample_data(physeqR)$InitialConc)) + 0.001 # math to make decontam work
sample_data(physeqR)$is.neg <- sample_data(physeqR)$Sample_or_Control == "Control"
...
ggplot(data=df.map, aes(x=Index, y=LibrarySize, color=Sample_or_Control)) + geom_point()

### ID contaminants by either prevalence or frequency
df.contam <- isContaminant(physeqR, method="either", neg="is.neg", conc="InitialConc",threshold=0.2)
phyN <- prune_taxa(!df.contam$contaminant, physeqR)
...
physeq0 <- subset_samples(physeqR,!sample_names(physeqR)%in%sample_names(phyN.pruS))
...
#track <- cbind(filtered, sapply(dadaFs, getN), sapply(dadaRs, getN), sapply(mergers, getN), rowSums(seqtab.noBim), rowSums(otu_table(phyN)), rowSums(otu_table(phyN.pruT)))
#colnames(track) <- c("input", "filtered", "denoisedF", "denoisedR", "merged", "nonchim", "Decontam", "noSingletons")
track <- cbind(filtered, sapply(mergers, getN), rowSums(seqtab.noBim), rowSums(otu_table(phyN)), rowSums(otu_table(phyN.pruT)))
colnames(track) <- c("input", "filtered", "merged", "nonchim", "Decontam", "noSingletons")
